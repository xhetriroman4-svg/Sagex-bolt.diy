...
export function createBranch(
  ...
  const targetSnapshotId = fromSnapshotId || currentSnapshotId.get() || existingBranches[currentBranch.get()]?.headSnapshotId;
...
export function getSnapshotDiff(snapshotId1: string, snapshotId2: string): {
  added: string[];
  modified: string[];
  deleted: string[];
} {
      ...
      if (lastSnapshot) {
        const hasChanges = Object.keys(fileMap).some((path) => {
          ...
          return !current !== !previous || 
            (current && previous && current.type === 'file' && previous.type === 'file' && 
...
export function addTag(snapshotId: string, tag: string): boolean {
  const snapshot = snapshots.get()[snapshotId];
  if (!snapshot || snapshot.tags.includes(tag)) return false;
